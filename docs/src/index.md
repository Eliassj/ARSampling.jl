```@meta
CurrentModule = ARSampling
```

# ARSampling

Documentation for [ARSampling](https://github.com/Eliassj/ARSampling.jl).
...
ARSampling.jl allows you to sample from unnormalized [logarithmically concave](https://en.wikipedia.org/wiki/Logarithmically_concave_function) distributions. In the following example we will sample from an unnormalized [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution).
...
Legend( # hide
    ...
    ["ARSampling.jl", "Distributions.jl"], # hide
